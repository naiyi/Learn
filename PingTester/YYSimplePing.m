...
typedef struct YYIPv4Header YYIPv4Header;

__Check_Compile_Time(sizeof(YYIPv4Header) == 20);
__Check_Compile_Time(offsetof(YYIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(YYIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(YYIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(YYIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(YYIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(YYIPv4Header, timeToLive) == 8);
__Check_Compile_Time(offsetof(YYIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(YYIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(YYIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(YYIPv4Header, destinationAddress) == 16);
...
- (void)didFailWithError:(NSError *)error {
    id<YYSimplePingDelegate>  strongDelegate;
...
- (NSData *)packetWithPingData:(NSData *)data {
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:YYICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:YYICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet timeToLive:(NSInteger *)timeToLive {
    ...
    NSUInteger                  result;
    const struct YYIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(YYIPv4Header) + sizeof(YYICMPHeader))) {
        ipPtr = (const YYIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
             ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(YYICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr timeToLive:(NSInteger *)timeToLive {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct YYICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);
        ...
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == YYICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == YYICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
static void STSocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge YYSimplePing *) info;
    assert([obj isKindOfClass:[YYSimplePing class]]);
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                ...
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        ...
                        if (self.addressStyle != YYSimplePingAddressStyleICMPv6) {
                    ...
                    case AF_INET6: {
                        ...
                        self.IPAddress = [NSString stringWithFormat:@"%s", s];
                        if (self.addressStyle != YYSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
