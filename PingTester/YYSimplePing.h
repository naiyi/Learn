...
@protocol YYSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, YYSimplePingAddressStyle) {
    YYSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    YYSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    YYSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface YYSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<YYSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) YYSimplePingAddressStyle addressStyle;
...
@protocol YYSimplePingDelegate <NSObject>
...
typedef struct YYICMPHeader YYICMPHeader;

__Check_Compile_Time(sizeof(YYICMPHeader) == 8);
__Check_Compile_Time(offsetof(YYICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(YYICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(YYICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(YYICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(YYICMPHeader, sequenceNumber) == 6);

enum {
    YYICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    YYICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    YYICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    YYICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
