...
+ (void)POST:(NSString *)URLString parameters:(id)parameters progress:(void(^)(NSProgress * uploadProgress))progress success:(void(^)(id responseObject))success failure:(void (^)(NSError * error))failure
{
    ...
    YYHTTPSessionManager *manager = [self createAFHTTPSessionManager];
...
+ (YYHTTPSessionManager *)createAFHTTPSessionManager
{
    YYHTTPSessionManager *manager=[YYHTTPSessionManager manager];
    manager.responseSerializer=[YYHTTPResponseSerializer serializer];
    manager.requestSerializer = [YYHTTPRequestSerializer serializer];
